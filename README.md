...
- **Later, when the need shows up.** A second AI model for independent cross-checks, the agent fleet, the overnight workhorse. Each is worth it only once you have enough going on that planning the work and waiting on it are themselves a cost.
...
- [Working with a co-author](./workflows/working-with-a-co-author.md) — when two people write a paper and both have an AI, the shared repository becomes the paper's home base; how to set the ground rules so the two assistants help each other instead of overwriting each other — and how to share the method without sharing the unfinished paper.
...
- [The terminal (optional)](./workflows/move-to-the-terminal.md) — you probably don't need it: the desktop app now runs hooks, skills, and scheduled tasks. What the terminal is still for, plus how to think about your context window.
...
- [An undo button for your whole vault](./workflows/an-undo-button-for-your-vault.md) — keep a running history so an edit (including an AI's) can be taken back, keep copies that don't fail together, and restore from them monthly to prove they work.
