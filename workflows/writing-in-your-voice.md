...
*What you'll need: a sample of your own writing for the AI to learn from — papers, essays, past emails, whatever format you're targeting. The fidelity check that protects meaning during a rewrite leans on a second AI model on the command line, run read-only (the maintainer uses the Codex CLI); worth setting up, but you can start with just the voice guide and add the check later.*
...
**Scale the guard to the stakes.** The maintainer uses three levels. A quick internal note gets a voice pass and a self-check — apply the guide, read it yourself, done. An external email or donor note with dates, names and commitments gets the mechanical number-and-name diff plus a short ledger of every commitment, checked before and after. Only a long or structured piece — a report, a chapter, web copy making claims — gets the full loop with an independent second model. Reserve the belt-and-braces for prose where a silently-drifted meaning would actually cost you.
