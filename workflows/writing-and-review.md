...
- **A second, independent model.** The reviewer is a *different* AI model from the one that wrote the draft, reached through its own command-line tool — the maintainer uses OpenAI's Codex CLI — and run **read-only**, so it can comment but can't silently edit your document. It's told to play hostile reviewer: not "is this good?" but "where will a real reader attack this, and why?" It returns specifics — the exact claim, the exact line, the objection — not vibes. For high-stakes work, run two such models and compare: each architecture spots different weaknesses, and anything both flag is almost certainly real. The independence is what gives this its value; a model grading its own homework mostly tells you it did well.
- **One frozen draft, one consolidated list.** Give every critic the same copy of the draft, at the same moment, and don't show one critic another's output — reviewers who can see each other converge, which looks like agreement but isn't. Then merge what comes back into a single ranked list, so you work through the findings once rather than re-reading the whole piece for each reviewer. Two rules make the merge trustworthy: nothing gets dropped on the way in (a finding only one critic raised still appears, just lower down), and where two critics genuinely disagree, keep both positions side by side as competing options rather than averaging them into something neither said. Those disagreements are usually the most informative thing the review produces.
- **You decide what to fix.** The review surfaces problems; acting on them is yours. The whole step only pays off if you actually work through what it finds rather than nodding past it. Keep a short record as you go of which findings you applied and which you deliberately let stand — it lets you stop and resume without re-litigating decisions, and it feeds the re-check below.

**Verify any citations against the real record.** Where the writing leans on published literature, every reference is checked against the scholarly databases — Semantic Scholar, CrossRef, and OpenAlex — and comes back as found, not-found, or ambiguous, with the supporting evidence. *Not-found means possibly fabricated.* AI models do invent realistic-looking references — plausible authors, a plausible year, a journal that exists, a paper that doesn't — and a flagged citation is held for your attention, never silently kept or silently deleted. And there's a deeper version worth reaching for once the existence check is solid: not just whether a paper exists, but whether it says what you claim. Where you actually hold the cited paper, its full text is pulled in so the reviewer checks your claim against the source itself — not the abstract, not its own memory — catching the reference that's real but misrepresents what it cites, which a pure existence check sails right past. (When the paper can't be retrieved, the reviewer falls back to the abstract and flags the gap rather than treating absence as confirmation.) This is worth building as its own small reusable check that the review step calls, so anything you write can lean on it.

**Re-read the draft after you've made the edits.** The review told you what was wrong with the draft you *had*; it says nothing about the draft you now have. Fixes introduce their own problems — a paragraph patched to answer one objection can contradict a claim three pages later, and an edit made to satisfy a critic can flatten a sentence into generic prose. So run one final reviewer over the edited document, and give it what actually changed: the before and after, plus the record of which findings you applied and which you let stand. Its brief is narrow — did each applied edit achieve what it was meant to, did any of them break something that was fine before, and does the piece still sound like you? This is the only pass that reads the document you're actually going to send.

**Polish last.** Once the substance holds, a light final pass cleans grammar, tightens wordy passages, and smooths tone — without flattening your voice into generic AI prose. A good polish step even scans for the tells of AI writing (overused stock words, formulaic three-part lists, an em-dash in every paragraph) and strips them out, so the result reads like you wrote it rather than a model. Keep this gentle and keep it last.
