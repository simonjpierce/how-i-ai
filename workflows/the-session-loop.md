...
**The handoff isn't only for the end of the session.** A long conversation gets less sharp as it fills, and — less obviously — steadily more expensive, because the AI re-reads the whole thing on every turn. So don't wait for it to fill up. A small check watches the conversation's size, and once it passes a set line (the maintainer uses about 30% of the window, then every further 10%), the AI raises it at the next natural pause with a one-key question: save and reset now? Say yes and it runs the light save described below, so the decisions and the current state are written into your notes, then hands you a ready-made command to compress the conversation around what you're doing next — and you carry straight on in the same session. (The cost side is covered in [managing your token budget](./managing-your-token-budget.md).) Left alone, the AI will eventually compress the conversation on its own priorities, and everything it decided to drop is gone unwritten. Choosing the moment yourself is the difference between a summary you wrote and a summary that happened to you.

**Make the mid-session save cheap.** The full closing pass does more than write the handoff — it also distils lessons into long-term memory, sweeps the related documents, and reviews what the session missed. That's worth paying once, at the end. Paying it every time you want a quick save is exactly what makes people stop doing quick saves and tell themselves they'll document later. So give it a light variant: bring the related documents current, write the handoff entry and the logs, save the work, and hand back the compress command — skipping only the heavier *learning* passes (distilling lessons into memory, the second sweep for anything the session missed). Keeping the documents current is the part a quick save must not skip: a decision that hasn't reached its project note is exactly what a compression loses. Two rules keep it safe — the full version stays the default, so you can never get the light one by accident; and the light one ends by naming what it *didn't* do, so a quick save is never mistaken for a finished close-out. Route by the cue rather than by feel: a long conversation, an imminent compaction, or a plain "save my progress" are all mid-session signals and belong on the light save; only a real goodbye earns the full pass. It helps to keep the heavy steps in a separate file the routine opens *only* for a genuine close-out — then the everyday save isn't carrying them around just to skip them.
