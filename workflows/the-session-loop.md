...
**The handoff isn't only for the end of the session.** An AI's working memory has a ceiling, and it gets noticeably less sharp as it fills. Watch the usage indicator, and when it's around half to sixty per cent full, treat that as the cue to wind down *this stretch of work*: run the light save described below — the handoff entry and the logs, not the full closing pass — so the decisions and the current state are written into your notes, then compress the conversation with a short instruction about what you're doing next, and carry straight on in the same session. Left alone, the AI will eventually compress the conversation on its own priorities, and everything it decided to drop is gone unwritten. Choosing the moment yourself is the difference between a summary you wrote and a summary that happened to you.

**Make the mid-session save cheap.** The full closing pass does more than write the handoff — it also distils lessons into long-term memory, sweeps the related documents, and reviews what the session missed. That's worth paying once, at the end. Paying it every time you want a quick save is exactly what makes people stop doing quick saves and tell themselves they'll document later. So give the closing routine a light variant: write the handoff entry and the logs, save the work, and skip the heavier learning and tidying passes. Two rules keep it honest — the full version stays the default, so you can never get the light one by accident; and the light one ends by naming what it *didn't* do, so a quick save is never mistaken for a finished close-out. Route by the cue rather than by feel: a long conversation, an imminent compaction, or a plain "save my progress" are all mid-session signals and belong on the light save; only a real goodbye earns the full pass. It helps to keep the heavy steps in a separate file the routine opens *only* for a genuine close-out — then the everyday save isn't carrying them around just to skip them.
