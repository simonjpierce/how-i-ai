...
**Use a quick save when you're carrying on.** A long conversation, an imminent compaction or a plain "save my progress" should bring the project notes current, write the handoff entry and the logs, save the work, and hand you the command to compress the conversation. Updating the project notes is the part a quick save must not skip: a decision left only in the chat can disappear in the compression. If a quick save costs as much as a full close-out, people stop doing them and tell themselves they'll document later — so keep it light.

**Use the full close-out when you're finishing.** Only a real goodbye earns it: on top of the quick save, it distils lasting lessons into long-term memory and sweeps for anything the session missed. Keep those heavier steps in a separate file the routine opens *only* for a genuine close-out, so the everyday save isn't carrying them around just to skip them. The full close-out stays the default when you say goodbye, and the quick save ends by naming what it skipped, so it's never mistaken for a finished close-out.
