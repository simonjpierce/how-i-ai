...
- **Check the branch before every commit.** Sessions and background jobs routinely leave a repo on a stale feature branch. A commit there "succeeds" and never reaches main. So: check the current branch first; if it's not main, cherry-pick to main, push main, and clean up. A hook can do the check for you. And never suppress a push's error output or chain a fallback onto it — one chained push swallowed a double rejection and the session reported success.
