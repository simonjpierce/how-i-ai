...
**Let it act without nagging you.** Before it starts, switch your AI to its most autonomous permission mode so it can install tools, create the vault, and write files without stopping for approval at every step — in the Claude desktop app, use the permission-mode selector in the prompt box and choose **Auto** (in the terminal version, press **Shift-Tab** to cycle to it). Otherwise setup stalls on a wall of yes/no prompts and feels broken when it isn't. You can dial it back any time.
...
<!-- model-box model=claude-opus-5-5 effort=high as_of=2026-09-23 -->
...
**What I use (as of 2026-09-23):** **Claude Opus 5.5, at *high* effort**, for all my interactive work. I moved to it from Claude Fable 5.1 the day it launched: Anthropic's own comparison put Opus 5.5 level with or ahead of Fable on every benchmark at a fraction of the cost per task, and it passed a source-checked research test that Fable failed, which matters for scientific work. Anthropic's default is *medium* effort and that is the sensible starting point; I run high and watch the quota. Background and overnight jobs run on Opus at high effort; my independent second model family is OpenAI's Codex. If Opus 5.5 isn't in your model picker, pick the most capable Claude your plan offers.
