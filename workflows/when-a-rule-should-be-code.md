...
- **Keep a firing ledger.** Wrap each hook in a tiny runner that counts silent fires and logs every warn, block or error with the session and tool. Without it, "which hooks have ever blocked anything?" is unanswerable, and dead hooks accumulate. With it, one had fired 348 times in a fortnight, every one a false positive after the harness changed — and was cut.
