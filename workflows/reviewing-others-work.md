...
The mirror case — reviewing *your own* draft before submission — is its own workflow: [reviewing your own manuscript](./reviewing-your-own-manuscript.md). The same multi-model pass and number-checking apply, but instead of a warm note for someone else you fold the fixes straight into your text, and add a [replication audit](./the-replication-audit.md) (is the code and data behind every figure and number actually in a repository someone could re-run?). Decide up front whose manuscript this is, because it changes the output (a note *for* an author, versus edits *into* your own draft).
