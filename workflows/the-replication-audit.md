...
**Check the code would actually run for someone else.** Beyond presence, a light cleanliness pass: are the dependencies stated, so someone could recreate the environment? Are there absolute paths hard-wired to your own machine that would break for anyone else? Is the order things must be run in clear? It won't re-run the whole analysis (often it can't), but it does try the cheapest step to confirm the thing actually runs, and it flags anything that would obviously stop an outsider cold.

**Check the Methods against the code.** Does the model actually include what the Methods section says it includes? This is the high-yield, often-missed check: a reviewer's "X is missing from the model" is settled by reading the code — and, if it matters, by switching the term on and off and re-running to see how far the result moves. A separate code review, told to hunt for bugs that would quietly corrupt a number, runs alongside.

**Handle sensitive data openly.** Some data can't be public — the precise locations of a threatened species, human-subject data, a partner's unpublished records. The answer is not to skip reproducibility but to be explicit about the carve-out: keep the repository private, or coarsen and hold back the risky detail, and *note what was held back and why*. Documenting a restriction doesn't make the result reproducible on its own, but it makes the limits clear: say what another researcher can reproduce with the available material, and how they can request access to anything withheld. An undocumented gap is just a hole.
