...
**A clear division of labour between the two.** Standing instructions hold what you *set on purpose*: who you are and your settled rules. Learned memory holds what was *learned through use*: the correction from last Tuesday, the gotcha to avoid next time. And memory is the last home for a rule, not the first. If a rule is mechanical enough for a small script to enforce, make it a script ([when a rule should be code](./when-a-rule-should-be-code.md)). If it's a step-by-step how-to, write it up as its own note and keep only a pointer. If it applies to one area of your work, put it in that area's instructions file. What's left — the learned corrections — goes in memory. Keeping each kind of rule in one place stops copies drifting into contradiction, which is the quiet way this kind of system rots.
