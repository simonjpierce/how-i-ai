...
**Cross-check.** Hand the source and the draft translation to two independent reviewers (the maintainer uses the Codex CLI plus a fresh instance of Claude, separate from the one that did the translating) and ask each to flag mistranslations, omissions, wrong numbers and inconsistent terms — a review, not a rewrite. Give the high-stakes passages (anything legal, any table of figures) a line-by-line check. When the two reviewers disagree, the source document is the arbiter — go back and read it. This is the same instinct as [surfacing conflicts](./surfacing-conflicts.md): two independent perspectives catch what one confident pass misses.
...
It does not produce a legally authoritative text. A careful AI translation is a working reference — excellent for understanding, briefing colleagues, and deciding what matters — but for anything binding, the original-language document remains the authority, and the translation should say so. It also doesn't remove the need for a human who knows the subject: the process catches dropped sections and drifting terms, but judging whether a subtle rendering is *right* in context is still yours. The verification layer raises your confidence; it doesn't hand you certainty.
...
This is a pattern, not a fixed pipeline. Which models translate and review, how fine-grained the sections are, how much of the document gets a line-by-line check — all yours. The durable idea is: *translate in sections against a locked glossary, have independent models check the translation against the source, and let the original settle every disagreement.* Paste this to your AI and build the version that fits the documents you work with.
