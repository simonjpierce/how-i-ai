...
- **Park in one move.** A single command composes the pieces: save the session state as usual; write or refresh the State of Play and set it to `paused — awaiting <condition>`; create a to-do in your task app whose notes *are* the pickup prompt, with a due date if the condition has one. Three owners, one composer — the maintainer rebuilt this after finding the park command had grown its own copy of the document logic.
