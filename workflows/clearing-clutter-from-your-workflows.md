...
- **Start from the provider's own audit.** Claude Code has a built-in `/checkup prompt-audit` that reads your instruction files and reports wording the current model no longer needs, rules that have drifted apart and files that contradict each other. It changes with each release, so reading its latest report before slimming a command means every pass benefits from Anthropic's current advice. The maintainer runs it weekly; its first run over about 80 commands found around 50 problems.
