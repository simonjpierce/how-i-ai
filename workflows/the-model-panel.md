...
*What you'll need: at least one panel member from a different model family, reachable from the command line — the maintainer uses OpenAI's Codex, a separate install and account. Further seats can be filled by fresh, independent instances of your main model. With only your main model you can still get value (run it twice in fresh, independent passes and merge), but at least one different model *family* on the panel is the real point, so flag the setup cost up front.*
...
- **Classifying a large pile of records (bulk coding).** You have hundreds or thousands of scrappy free-text entries — field notes, survey comments, case descriptions — that need sorting into categories. Every model gets the same written scheme and the same records, independently. Where all of them land on the same answer, accept it. Where they don't, the record is flagged with what each model said, and a person decides. It's the same independence logic, used to *find the hard cases* rather than to answer one question.

## How it runs

- **The panel — different families, in parallel, blind to each other.** Three independent reviewers read the same frozen input at once: your main model (Claude), a different model family reached from the command line (the maintainer uses Codex), and a third seat filled by another independent instance of the main model in its own fresh context. "Command line" just means the text terminal the agent already drives; the extra models are run **read-only** there — they can read everything and comment, but can't change a file. A cross-family member also runs on its *own* account and quota, so leaning on it doesn't eat into your main model's budget. Keeping them blind to one another is the whole point — three genuinely independent reads, not an echo.
...
- **Group the disagreements before you look at them.** Don't hand a person a thousand flagged records. Sort the flags by *why* the models disagreed, and a pattern usually jumps out: one ambiguity in your scheme, hit over and over, is generating most of the flags. That isn't the models failing — it's a question your written scheme never answered. Get the expert to settle it once as a rule, apply it to every record it covers, and only then work through what's genuinely left. A handful of rule decisions usually clears the large majority. Write each rule into the scheme as you make it, so the next run doesn't reopen it. For bulk coding, write the scheme down first as a short document all the models get, include a couple of dozen worked examples from decisions the expert has already made on their own data, spot-check fifty of the unanimous rows at random, and put a link to the source record on every flagged row so the human can check it in one click.
...
- **It must not resolve the ambiguity for you.** Give the models an explicit "unsure" option and instruct them to use it whenever the source text doesn't actually say. An assistant without that option will fill the gap plausibly and you will never see it happen — a plausible invented value is worse than a blank, because it looks like data. If a call really needs a human looking at a photograph, say so up front and keep the models out of it. Their job is to sort the easy majority and hand you a clean list of the hard ones.
