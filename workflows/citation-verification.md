...
**Check your own library first** *(if you keep one).* A reference whose DOI matches a paper in your curated [reference mirror](./the-reference-library.md) can be treated as found — and if the draft's author or year disagrees with your library's record, that's flagged as a conflict worth a look. That clears the bulk of a typical reference list cheaply and leaves the unfamiliar ones for external lookup. One caveat: your library is only as good as its metadata. Records without a DOI — theses, book chapters, reports — are exactly the ones no automated check has looked at, and a hand-entered record can carry a wrong year, so for anything high-stakes either re-confirm against the databases or mark it *found in your library, not externally re-checked*.
...
- **Not found** — no matching paper returned. After ruling out a parsing slip, this means *possibly fabricated*, and it's the one that matters most. Build the checker so a database request that simply *failed* — a network error, a rate limit — is reported as its own "couldn't check" verdict rather than as not found; otherwise an outage looks exactly like a fabricated reference.
