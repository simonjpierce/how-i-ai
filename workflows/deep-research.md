...
*What you'll need: the "search your own sources first" step works best with a searchable local mirror of your reference manager — a small one-time setup your AI can walk you through, and you can start without it and add it later. A second AI model adds breadth but is optional; with only one model it still searches your own sources first, then the web, and leans harder on the source and reference-database checks (which need no account — those databases are open).*
...
**Search your own sources first.** Before touching the open web, it searches the material you already trust — your notes, your saved papers, your reference library. The non-obvious requirement here is that those sources have to be *searchable on your own machine*, not locked inside a cloud account: the maintainer keeps a local mirror of his reference manager (Paperpile, though Zotero or any manager works the same way) — the abstracts and metadata of everything he's saved, each linked to its PDF, plus the full extracted text of every paper that has a PDF, as a searchable sidecar — so the AI can search across the whole library's full text and open the underlying paper when a hit is worth a closer look. That full-text layer was built once in a batch (cheap with a plain text extractor) and is topped up automatically each night as new papers arrive. ([The reference library](./the-reference-library.md) is how you build that mirror.) Trusted sources needn't be academic: a curated collection of practitioner material — talks, interviews, transcripts from the people who actually do the work — is worth searching the same way for questions whose answer lives in practice rather than in journals, and it's kept as a *separate* pass, because finding plenty of practitioner opinion doesn't reduce the reading owed to the published evidence. It also searches your own correspondence — email and team chat — for unpublished results, decisions and what colleagues have said, because much of what an organisation knows never makes it into a paper; relevant correspondence goes into the briefings for the research models themselves, but it's removed from the optional brief you'd paste into a browser-based research product. That grounding is what makes the report *yours* and not a generic web summary — it builds on what you already know, and can flag where a fresh web source contradicts it.

**Gather widely — and, ideally, with more than one model.** It runs several searches in parallel, one per sub-question, fetching and reading the promising results rather than skimming snippets. Every claim it records is tied to the source it came from — no free-floating assertions. Where a second AI is available on the command line (the maintainer uses the Codex CLI, run **read-only** so it can't change anything), that model researches the same question in parallel; the two often surface complementary findings, and where they agree it's a useful signal — not proof, but a cue worth more checking.

There's also a third source of breadth that needs no command-line setup at all: the **browser-based deep-research products** (Gemini, ChatGPT, and Claude all offer one). Once the internal report is drafted, the workflow can hand you a self-contained research brief to paste into one or more of those, and then reconcile whatever comes back against its own findings — every claim and citation from an external product gets the same verification as everything else, because those products hallucinate references too. This is optional and deliberately a *you-in-the-loop* step: nothing is sent to a third-party service without your say-so, and the brief is scrubbed of anything private before it leaves your machine.

**Synthesise without smoothing.** It pulls everything into one report, attributing claims at the level of the individual finding, not the paragraph. Where sources disagree it shows *both* positions and their evidence instead of silently picking a winner. Consequential claims carry a confidence note — well-corroborated, single-source, or uncertain — so you can see how much weight each one bears.
...
- *Nothing the second model found gets dropped.* Where a second model researched in parallel, the draft is checked against its findings, so nothing it turned up is quietly lost in the synthesis. If no second model was available, the report says so; it never pretends a review happened.

**Argue with the merged report — when there is one.** Once outside reports have been folded in, the merged document gets one adversarial read before it's delivered: two independent reviewers, working from the finished report rather than from the sources, hunting the failures verification structurally can't catch — an absolute claim that should be hedged, two different things conflated into one, a summary table that contradicts the narrative, a recommendation the findings don't actually support. Where both reviewers land on the same problem independently, that's the shortlist worth acting on. What happens next depends on who the report is for: for internal work the clear-cut fixes are applied and the rest noted; for anything going outward the findings come to you to rule on, because a document headed for other people shouldn't be silently rewritten. A plain run with no external passes skips this — it earns its cost exactly where several sources have been merged, and the seams are where the errors hide.

**Deliver.** The finished report — with its citations, its flagged uncertainties, and its verification notes — is saved into your notes so it compounds with everything else you know, rather than scrolling out of a chat. When the question drew on published literature, it also hands you a short, deduplicated list of the papers it cited that *aren't yet in your own library* — ranked by how much they matter to the question — so each run grows your trusted sources by relevance instead of leaving you to diff a bibliography by hand.
