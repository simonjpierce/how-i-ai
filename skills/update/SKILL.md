...
10. **Code backup check**. If the session created or modified scripts, analysis code, or data outputs:
   ...
   - **`~/repos/mmf-claude-code/` specifically**: if the session touched any skill, template, or guide that's mapped in `sync-from-vault.sh`, run the sync flow with the course sweep wrapper rather than a plain `git add` / `commit` / `push`:
     ```bash
     cd ~/repos/mmf-claude-code
     PRE_SYNC_HEAD=$(git rev-parse HEAD)
     ./sync/sync-from-vault.sh --commit
     POST_SYNC_HEAD=$(git rev-parse HEAD)
     ```
     Then follow `~/repos/mmf-claude-code/sync/course-sweep-protocol.md` with `CALLER="update"`, the captured HEADs, and `REPO_ROOT=~/repos/mmf-claude-code`. The sweep proposes lesson edits for any modified starter skill, scaffolds lessons for new local skills that should ship, and archives lessons for retired skills. **Exits silently** if nothing in the sweep's signal set changed. Surface the sweep summary in the Phase 4 Summary if it ran.
