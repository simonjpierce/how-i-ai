...
- **The distillation pass (periodic, scheduled).** The repayment. Each command is slimmed and checked on its own. When several are due at once, finish one completely (both checks below included) and read its result before running the rest in parallel, so a systematic slip is caught once rather than repeated across the batch. The maintainer schedules it: a weekly health check ranks candidates and fixes mechanical faults, and every fortnight the top one is slimmed. The human doesn't approve each cut — the do-no-harm check below is the gate, and only a disagreement between its passes reaches them.

## Start from the provider's own audit

Before sorting anything, get the model maker's current view of what goes stale. In Claude Code that is the built-in `/checkup prompt-audit` (also `/doctor prompt-audit`): it reads your `CLAUDE.md` files, skills and subagents and reports instructions the current model no longer needs, such as coaching aimed at older models, capital-letter pressure, workarounds for limits that have since been lifted, duplicated rules that have drifted apart, and files that contradict each other. It only reports; it changes nothing. Because it is updated with each Claude Code release, reading its latest report first means every slimming pass benefits from the provider's current advice without you tracking it yourself. Treat its findings for the command you are slimming as part of the job, and look for the same patterns across the whole body, not just the lines it cites. The maintainer runs it once a week and re-runs it before a pass when the report is more than a week old or was made by an older Claude Code version. The first run over a setup of about 80 commands found around 50 problems, and a second run the next day found stale wording in commands that had been slimmed only the day before, which is why it is now an input rather than an occasional extra.
...
If a rule has no live home, put it back in the body (or an appendix the body points to at the right moment) and run the checks again. The semantic pass asks two questions, not one: *any lost rules?* and *any changed meaning* — including whether a relocated rule will actually be loaded when it's needed. Explain any deliberate removal (a dead flag, a retired script). Only a disagreement that needs a design or scope decision goes to a human; never silently drop. In practice the semantic pass earns its keep — on a real run of this over a dozen commands, it caught genuine dropped rules in about half of them that the mechanical pass alone missed.
