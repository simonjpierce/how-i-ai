...
**Put a one-line focus pointer near the top of every entry.** A `> read this first` banner names the single immediate next action, with enough context to take it. This is the insurance against compaction: if a long session's memory gets summarised away mid-thread, the next start lands on the right thing instead of the top of a generic log. With several sessions running, the resuming AI looks for the entry carrying *its own* session marker — the newest entry may belong to another conversation. Cheap when it's not needed, decisive when it is.

**One caution with parallel sessions.** If you run more than one AI session at once, two of them can write to the handoff log between one's read and its write — the second write clobbers the first. The robust fix is a small helper that holds a shared file lock while it reads the latest log, inserts the entry, and saves the result atomically — and every session writing the log uses that same lock. Reading and writing in one script is not enough on its own, and an edit anchored on a line that may have moved is worse. Tag each entry with a short session marker so a concurrent run knows which entries are *yours* to update versus a sibling's to leave alone.
