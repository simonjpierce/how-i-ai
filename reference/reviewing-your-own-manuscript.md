...
- **A second model (and ideally a third pass), read-only, on the command line.** Different model families spot different weaknesses — anything two reviewers independently flag is almost certainly real. The Codex CLI is strongest on statistics and internal-consistency arithmetic; a further independent pass (the maintainer uses a fresh instance of the main model in its own clean context) on an open-ended "fresh eyes" framing catches things a structured rubric misses.
