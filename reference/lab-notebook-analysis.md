...
**No quiet exceptions.** The maintainer's gate has no opt-out: the only relief is a step with no output file to check against, recorded as *unverifiable* rather than waved through. (A lighter exploratory mode is a reasonable adaptation, provided every skip is logged with its reason.) And at the moments where a result really matters — settling what kind of model the data and the biology allow, choosing between model families, after a change of direction — offer a heavier check: three different models read the same material separately, told to find what's wrong, and the main session reconciles them. Offer it, don't run it automatically; a heavy pass on every step just trains you to tune it out.
