...
Maintainer's settings: threshold 300k, step 100k. The message is an instruction, not information: *"Context is ~340k tokens and every turn re-reads all of it. At the next natural break run the checkpoint command without asking, then hand back the compaction command. Do not interrupt a task mid-step for this."* The advisory version ("mention this once") was measured for a week and did nothing — 59 of 97 sessions crossed 300k and ran a median 158 more turns; 17 compacted. Instructing the model changed that; asking the human to remember did not. Fail-open: any error in the hook exits 0 silently.
